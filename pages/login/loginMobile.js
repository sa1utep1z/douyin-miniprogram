...
Page({
  ...
  onLogin: function () {
    ...
    if (phone.length !== 11) {
      tt.showToast({ title: '请完善下手机号', duration: 2000, icon: 'none' });
      return;
    }
    if (smsCode.length !== 6) {
      tt.showToast({ title: '请输入完整的验证码', duration: 200 });
      return;
    }
    ...
    tt.login({
      success: (res) => {
        const recommendId = tt.getStorageSync('recommendId') || '';
  ...
  appLogin: async function (params) {
    ...
    tt.setStorageSync('mobile', res.data.mobile);
    tt.setStorageSync('userId', res.data.userId);
    tt.setStorageSync('token', res.data.jwt);
  ...
  getGrapgCode: async function () {
    ...
    tt.request({
      ...
      success: (res) => {
        let url = 'data:image/png;base64,' + tt.arrayBufferToBase64(res.data);
        this.setData({
          grapgCodeUrl: url
        });
      }
  ...
  onDialogComfirm: function () {
    ...
    if (validationCode.length !== 4) {
      tt.showToast({ title: '请输入图形验证码', duration: 2000, icon: 'none' });
      return;
    }
    this.setData({
      showImageCode: false
