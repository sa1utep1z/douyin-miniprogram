...
Component({
  ...
  methods: {
    fetchSignInfo: async function () {
      ...
      // } catch (error) {
      // }
    }, onShowChange: function (value) {if (value) {this.fetchSignInfo();}}, onIdChange: function (value) {}, handleCancel: function (e) {this.setData({ smsCode: '' });
      this.triggerEvent('showSignInfo', false);
    },
    handleConfirm: async function (e) {
      ...
      if (name && phone && smsCode && phone.length === 11 && smsCode.length === 6) {
        ...
        tt.removeStorageSync('recommendId');
        this.setData({
          smsCode: ''
        });
        this.resetTimer();
